 ...
 * IDENTIFICATION
 *		  contrib/odbc_fdw/odbc_deparse.c
 *
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
 ...
 */
void
odbc_classifyConditions(PlannerInfo *root,
				   RelOptInfo *baserel,
				   List *input_conds,
				   List **remote_conds,
				   List **local_conds)
					...
					foreign_loc_cxt *outer_cxt)
{
		...
		case T_FuncExpr:
			{
				 ...
				 */
				if (!odbc_is_shippable(fe->funcid, ProcedureRelationId, fpinfo, ProcedureObj))
					return false;
		...
		case T_DistinctExpr:	/* struct-equivalent to OpExpr */
			{
				 ...
				 * function is too.)
				 */
				if (!odbc_is_shippable(oe->opno, OperatorRelationId, fpinfo, OperatorObj))
					return false;
		...
		case T_Aggref:
			{
				...
				if (!odbc_is_shippable(agg->aggfnoid, ProcedureRelationId, fpinfo, AggObj))
					return false;
				...
				if (agg->aggorder)
				{
					...
					foreach(lc, agg->aggorder)
					{
						...
						if (srt->sortop != typentry->lt_opr &&
							...
							!odbc_is_shippable(srt->sortop, OperatorRelationId,
										  fpinfo,  OperatorObj))
							return false;
					}
				}
	 ...
	 */
	if (check_type && !odbc_is_shippable(exprType(node), TypeRelationId, fpinfo, DataTypeObj))
		return false;
 ...
 */
extern void
odbc_deparseSelectStmtForRel(StringInfo buf, PlannerInfo *root, RelOptInfo *rel,
	...
	*retrieved_attrs = NIL;
						  ...
						  attrs_used))
		{
			if (!first)
				...
				ereport(ERROR, (errcode(ERRCODE_FDW_ERROR),
						errmsg("RETURNING not supported")));
			}
					  ...
					  attrs_used))
	{
		ereport(ERROR, (errcode(ERRCODE_FDW_INVALID_DATA_TYPE),
				errmsg("ctid columns are not supported")
                                                        ));
	}
	...
	{
		ereport(ERROR, (errcode(ERRCODE_FDW_INVALID_DATA_TYPE),
				errmsg("oid columns are not supported")
                                                        ));
	}
...
extern const char *
odbc_get_jointype_name(JoinType jointype)
{
	switch (jointype)
	{
		case JOIN_INNER:
					  ...
					  bool use_alias, List **params_list)
{
	...
	if (IS_JOIN_REL(foreignrel))
	{
		...
		appendStringInfo(buf, "(%s %s JOIN %s ON ", join_sql_o.data,
						 odbc_get_jointype_name(fpinfo->jointype), join_sql_i.data);
				   ...
				   bool make_subquery, List **params_list)
{
	...
	if (make_subquery)
	{
		...
		appendStringInfoChar(buf, '(');
		odbc_deparseSelectStmtForRel(buf, root, foreignrel, NIL,
 ...
 */
void
odbc_deparseInsertSql(StringInfo buf, PlannerInfo *root,
				 Index rtindex, Relation rel,
 ...
 */
void
odbc_deparseDirectUpdateSql(StringInfo buf, PlannerInfo *root,
					   Index rtindex, Relation rel,
					   List *targetlist,
					   List *targetAttrs,
					   List *remote_conds,
 ...
 */
void
odbc_deparseDirectDeleteSql(StringInfo buf, PlannerInfo *root,
					   Index rtindex, Relation rel,
					   List *remote_conds,
					   List **params_list,
					   List *returningList,
 ...
 */
void
odbc_deparseStringLiteral(StringInfo buf, const char *val)
{
	const char *valptr;
...
deparseConst(Const *node, deparse_expr_cxt *context, int showtype)
{
	...
	switch (node->consttype)
	{
		...
		case CSTRINGARRAYOID:
			/* replace {}  to quote array*/
			extval[0] = ' ';
			extval[strlen(extval)-1] = ' ';
			appendStringInfo(buf, "%s", extval);
			break;
		default:
			odbc_deparseStringLiteral(buf, extval);
			break;
	}
...
deparseScalarArrayOpExpr(ScalarArrayOpExpr *node, deparse_expr_cxt *context)
{
	...
	if (node->useOr) {
		switch (node->opno) {
			...
			case 1120: /* float48eq */
			case 1130: /* float84eq */
			case 1320: /* timestamptz_eq */
			case 1752: /* numeric_eq */
			case 1784: /* biteq */
			case 1862: /* int28eq */
			case 1868: /* int82eq */
			case 1955: /* byteaeq */
			case 2060: /* timestamp_eq */
			case 2347: /* date_eq_timestamp */
			case 2360: /* date_eq_timestamptz */
			case 2386: /* timestamptz_eq_date */
			case 2536: /* timestamp_eq_timestamptz */
			case 2542: /* timestamptz_eq_timestamp */
			case 2972: /* uuid_eq */
...
appendOrderByClause(List *pathkeys, deparse_expr_cxt *context)
{
	...
	foreach(lcell, pathkeys)
	{
		...
		em_expr = odbc_find_em_expr_for_rel(pathkey->pk_eclass, baserel);
		Assert(em_expr != NULL);
